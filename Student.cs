...
namespace FacultyManagementSystem
{
    public partial class Student : UserControl
    {
        ...
        private DataTable LoadUserTable()
        {
            ...
            DataTable dt = new DataTable();
            string query = "select student.stud_id,stud_courses.course_code, stud_courses.grade,stud_courses.year,stud_courses.semester from student,stud_courses where student.stud_id = stud_courses.stud_id";
            using (SqlCommand cmd = new SqlCommand(query, con))
            {
                con.Open();
                SqlDataAdapter da = new SqlDataAdapter(cmd);
        ...
        {
            con.Open();
            string q = "insert into stud_courses (stud_id,course_code,year,semester) values (@id,@code,@year,@sem)";
            ...
            cmd.Parameters.AddWithValue("@sem", semes.Text.Trim());
            cmd.ExecuteNonQuery();
            con.Close();
            dataGridView1.DataSource = LoadUserTable();
            dataGridView2.DataSource = LoadUserTable1();
