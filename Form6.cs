...
namespace FacultyManagementSystem
{
    public partial class Form6 : Form
    {
        ...
        public DataTable loaduser()
        {
            DataTable dt = new DataTable();
            con.Open();
            string q = "select stud_courses.stud_id,stud_courses.course_code,stud_courses.grade,student.major,student.depart_code " +
                "from stud_courses join student on stud_courses.stud_id = student.stud_id where student.stud_id = @id";
            SqlCommand cmd = new SqlCommand(q, con);
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            cmd.Parameters.AddWithValue("@id", _id);
            cmd.ExecuteNonQuery();
